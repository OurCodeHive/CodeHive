...
const StudyGroupMemberList = ({studyinfoId, closePop} : {studyinfoId : number, closePop: () => void}) => {
    const param = {
        study : studyinfoId,
        page : 0,
        size : 10
    ...
    return (
        <div className="col-12">
            <ul className={`col-12 mb30 ${Style.member_list_con}`}>
                {userList.length > 0 ? userList.map((item, index) => <UserListItem key={index} item={item} studyinfoId={studyinfoId} refreshList={refreshList} />) : <div>등록된 멤버가 없습니다</div>}
            </ul>
